...
class RANSACFlatPlane:
    ...
    def fit(
        ...
        mask_roi = (pc[:, 0] ** 2 + pc[:, 1] ** 2 <= R**2) & (pc[:, 2] >= zmin) & (pc[:, 2] <= zmax)
        pc_roi = pc_hom[mask_roi]

        assert pc_roi.shape[0] >= 3, "Insufficient points in ROI to fit plane!"

        # Vectorized RANSAC Sampling.
        T, M = self.params["num_trials"], pc_roi.shape[0]

        # Sample triplets.
        ids = torch.randint(low=0, high=M, size=(T, 3), device=dev, generator=generator)
